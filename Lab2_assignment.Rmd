...
3.  Pick a neighborhood of the City to map.
    ...
    Use the [PHL Track Explorer](c) to get the `GEOID10` number from each parcel and add them to the `myTracts` object below.
...
ggplot()+
  geom_sf(data = acsTractsPHL.2019.sf, aes(fill = Percent_Natural),
          color = "transparent") +
  geom_sf(data = acsTractsPHL.2019.sf %>%
            filter(Neighborhood == "SOUTH PHILLY") %>%
            st_union(),
          color = "white",
          fill = "transparent") + 
  ...
  labs(
    title = "South Philadelphia Percent Natural Born Asians",
    subtitle = "Percent population of Asian Population that is domestic",
